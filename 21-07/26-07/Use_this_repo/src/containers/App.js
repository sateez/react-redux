...
class App extends Component {
  ...
  changeScrollIndicator(onLoad) {
    ...
    var percent = ((wraperTop+wrapperHeight)/mbodyHeight  ) * 100;
    ...
    if (wraperTop>=100) {
      ...
      if (window.innerWidth > 1024) {
        ...
        //setTimeout(function() {
          if ($('.navbar-collapse.collapse').css('height') !== "1px" && ReactDOM.findDOMNode(this.refs.tButton).offsetHeight===0) {            
  ...
  render() {
    // let fixedCls = classNames({
    //   "":(document.body.scrollTop<=100 || window.innerWidth <= 1024),
    //   "fixed":(document.body.scrollTop>100 && window.innerWidth > 1024)
    // })
    return (
        <Grid className="scroll-body" >
          <Row>
            <Navbar inverse fixedTop>
              ...
              <div id="scrollIndicator" ref="scrollIndicator"></div>
            </Navbar>
          </Row>
          <Row>
          <Col xs={3} md={3} lg={3}>
