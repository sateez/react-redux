...
import css from '../css/primaryNavigation.css'
...
$(function(){
  $('.sideMenuHeading .cls-1').addClass('sideMenuHeadingFill');
  $('.tealChevron .cls-1').addClass('tealChevronFill');
})
...
$(document).on('click','.subMenulist',(e) => {
  ...
  target.find('.subMenuSubList').toggle('slide',{direction:'right'});
...
$(e.target).find('span').addClass('fillWhite').removeClass('lightgray');
})
...
<div className="sidemenuWrapper"> {MENUS.map(SubMenuItem)}
</div>
</div>
